...
int sputs(const char* str, int s) @nogc nothrow {
	char c = 0xFF;
	int n = 0;
...
int sockprintf(int s, const char* fmt, ...) nothrow {
	va_list args;
...
int conn(const char* host, int port, char[4] ipbuf) {
	version (HAVE_IPV6) {
	...
	version (HAVE_IPV6) {
		...
		for (res = res0; res; res = res.ai_next) {
			...
			getnameinfo(res.ai_addr, res.ai_addrlen, hbuf.ptr, hbuf.sizeof, null, 0, 0);
		...
		memcpy(ipbuf.ptr, &sa.sin_addr, 4);
	}
